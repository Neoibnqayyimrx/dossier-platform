...
- **The Application entity.** A dossier now belongs to an **organization**
  — colleagues share it, an org admin adds members, and a platform
  super-admin administers accounts across organizations without reading
  any of their dossiers (`docs/decisions/0004-organization-tenancy.md`).
  Still ahead: the agency-assigned application number lives on `Project`
  until an `Application` entity exists to hold it.
