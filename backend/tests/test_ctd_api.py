...
async def _seed_project(session_factory, buggy: bool = False) -> uuid.UUID:
...
async def test_build_ctd_returns_a_manifest_for_a_clean_project(client, session_factory):
    project_id = await _seed_project(session_factory, buggy=False)
    resp = await client.post(f"/projects/{project_id}/build/ctd")
...
async def test_build_ctd_409s_when_validation_has_unresolved_errors(client, session_factory):
    project_id = await _seed_project(session_factory, buggy=True)
    resp = await client.post(f"/projects/{project_id}/build/ctd")
    assert resp.status_code == 409


async def test_build_ctd_missing_project_404s(client):
    resp = await client.post("/projects/00000000-0000-0000-0000-000000000000/build/ctd")
